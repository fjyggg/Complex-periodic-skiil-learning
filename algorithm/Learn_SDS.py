...
class LearnSds:
    def __init__(self, lf_learner, grad_model, ods_learner):
        '''
        Initializing the Stable ADS
        '''
        self.lf_learner = lf_learner
        self.ods_learner = ods_learner
        self.grad_get = grad_model
        self.d_x = lf_learner.d_x

        self.b = 1
        self.max_v = 0.1

    def predict(self, x, func_rho, P=None, r_thres=0.0, eta=0.1):
        '''
        ...
        if x_P_norm == 0.0:
            ...
            dvdx = self.grad_get.get_dvdx(self.lf_learner, x_set_tensor)
    ...
    def show_learning_result(self, func_rho=None, plot_handle=None, x0s=None, P=None, r_thres=0.0, eta=0.1, stream_flag=True, energy_flag=False):
        if func_rho is None:
            func_rho = self.func_rho

        x_set = self.lf_learner.x_set
        x_set = x_set.detach().numpy()
        ...
        if stream_flag is True:
            for i in range(length_x2):
                for j in range(length_x1):
                    x = np.array([x1[j], x2[i]])  # 这里的数据要转换成torch方便计算梯度

                    desired_v, u = self.predict(x, func_rho, P=P, r_thres=r_thres, eta=eta)
        ...
        plot_handle.scatter(x_set.reshape(-1, self.d_x)[::4, 0],
                            x_set.reshape(-1, self.d_x)[::4, 1], c='red', alpha=1.0, s=mark_size,
        ...
        x0s = x_set[0, :]
        # x0s = x_set[0, :] - np.array([0,0.2])

        self.plot_repro(x0s, func_rho=func_rho, plot_handle=plot_handle, P=P, r_thres=r_thres, eta=eta)

        if show_flag is True:
            plot_handle.show()

    def plot_repro(self, x0, func_rho, plot_handle=None, P=None, r_thres=0.0, eta=0.01):
        x = x0
        period = 1e-2
        steps = int(40 / period)
        x_tra = [x]
